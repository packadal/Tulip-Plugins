...
void QPropertyTest::setNodeValueTest() {
	...
	_engine->evaluate(QString::fromStdString(
			"var g = newGraph(); storeGraph(g);"));
	...
	_engine->evaluate(
			"var realValue = new QColor(11, 12, 13, 0); property0.setNodeValue(node, realValue)");
	handleError();
	...
	CPPUNIT_ASSERT(value == result);
	_engine->evaluate(
			"var realValue1 = new QGraph(); realValue1.addNode(); property1.setNodeValue(node, realValue1)");
	handleError();
	...
	_engine->evaluate(
			"property4.setNodeValue(node, \"an other string to test\");");
	handleError();
...
void QPropertyTest::eraseNodeTest() {
	...
	_engine->evaluate(
			"property.erase(node);var value = property.getNodeStringValue(node); storeString(value);");
